...
// ЯВНЫЕ ЗНАЧЕНИЯ вместо process.env (чтобы избежать проблем)
const getDbConfig = () => {
  const config = {
...
export async function saveUserSolution(
  ...
  try {
    console.log(`💾 Сохраняю решение для кейса ${caseId}`);
    
    // Проверяем существование таблицы USER_SOLUTIONS
    try {
      // Используем строки 'TRUE'/'FALSE' для Firebird BOOLEAN
      const isCorrectValue = isCorrect ? 1 : 0; // Используем 1/0 для совместимости
      
      await executeQuery(
        `INSERT INTO USER_SOLUTIONS (CASE_ID, USER_QUERY, IS_CORRECT, CREATED_AT) 
         VALUES (?, ?, ?, CURRENT_TIMESTAMP)`,
        [caseId, userQuery, isCorrectValue]
      );
      
      console.log(`✅ Решение для кейса ${caseId} сохранено в USER_SOLUTIONS`);
      
    } catch (tableError: any) {
      // Если таблицы нет, логируем и продолжаем
      console.warn(`⚠️ Таблица USER_SOLUTIONS не найдена: ${tableError.message}`);
      console.log(`ℹ️ Решение для кейса ${caseId} не было сохранено (таблица отсутствует)`);
    }
    ...
    console.error(`❌ Ошибка при сохранении решения для кейса ${caseId}:`, error.message);
    // Не выбрасываем ошибку, чтобы не ломать пользовательский опыт
  }
}
...
function getDefaultQuery(id: number): string {
  switch(id) {
    ...
    case 2: return "SELECT employee_name, salary FROM employees WHERE salary > (SELECT AVG(salary) FROM employees);";
    case 3: return "SELECT * FROM network_logs WHERE ip_address = '192.168.1.100' AND EXTRACT(HOUR FROM timestamp) > 18;";
...
function getDefaultExplanation(id: number): string {
  switch(id) {
    ...
    case 3: return "Фильтруйте записи по IP-адресу и используйте функцию EXTRACT(HOUR FROM ...) для проверки времени.";
